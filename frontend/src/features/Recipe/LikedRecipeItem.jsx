...
export default function LikedRecipeItem({
  data,
  isEdit = false,
  isAdmin = false,
}) {
  ...
  const handleShowMore = () => {
    navigate(`/show/${data?.recipe?.id}`);
  };

  const handleLike = () => {
    toggleLike(data?.recipe?.id);
  };

  const handleUpdate = () => {
    navigate(`/edit/${data?.recipe?.id}`, {
      state: {
        data,
      },
  ...
  const handleDelete = () => {
    deleteRecipe({ id: data?.recipe?.id });
  };
    ...
    <div className="rounded-lg bg-gradient-to-br from-slate-950 via-gray-900 to-black p-6 shadow-lg transition-transform hover:scale-105 hover:shadow-xl">
      ...
      <div className="mb-4 flex items-center justify-between">
        ...
        <p className="text-sm font-medium text-gray-400">
          ...
          {data.recipe?.createdBy?.name || "Anonymous"}
        </p>
      ...
      <p className="mb-4 text-sm text-gray-300">
        {data.recipe?.description?.length > 100
          ? `${data.recipe.description.slice(0, 100)}...`
